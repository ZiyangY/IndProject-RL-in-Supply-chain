...
class Actor(nn.Module):
    def __init__(
        ...
        if self.recurrent:
            ...
            self.l1 = nn.GRU(state_dim, hidden_dim, batch_first=True)
        else:
...
class DDPG(object):
    def __init__(self, state_dim, action_dim, max_action, hidden_dim, discount,
        tau, actor_lr, critic_lr, recurrent_actor,recurrent_critic ):
    ...
    def train(self, replay_buffer, batch_size):
        ...
        #         print("NONE, ", name)
        #     else:
        #         grads.append(np.linalg.norm(param.grad.cpu()))
        #     print("-------------------------")

        #    #  grads.append(param.grad.view(-1))
        # # grads = torch.cat(grads)
        # grads.append(np.linalg.norm(grads))
        # print(grads)
        grads = []
        for name, param in self.critic.named_parameters():
        # for name, param in self.critic.parameters():
            if param.grad == None: # LayerNorm's gradient = None
                pass
            elif name[-1] == "t": #get the weight for each layers
                grads.append(np.linalg.norm(param.grad.cpu())) # calculate norm
