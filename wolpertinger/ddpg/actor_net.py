...
class ActorNet:
    ...
    def __init__(self, num_states, num_actions):
        ...
        with self.g.as_default():
            ...
            self.q_gradient_input = tf.compat.v1.placeholder("float", [None, num_actions])
            ...
            self.optimizer = tf.compat.v1.train.AdamOptimizer(LEARNING_RATE).apply_gradients(
    ...
    def create_actor_net(self, num_states=4, num_actions=1):
        ...
        N_HIDDEN_1 = 400
        N_HIDDEN_2 = 300
        actor_state_in = tf.compat.v1.placeholder("float", [None, num_states])
        W1_a = tf.Variable(tf.random.uniform(
            [num_states, N_HIDDEN_1], -1 / math.sqrt(num_states), 1 / math.sqrt(num_states)))
        B1_a = tf.Variable(tf.random.uniform(
            [N_HIDDEN_1], -1 / math.sqrt(num_states), 1 / math.sqrt(num_states)))
        W2_a = tf.Variable(tf.random.uniform(
            [N_HIDDEN_1, N_HIDDEN_2], -1 / math.sqrt(N_HIDDEN_1), 1 / math.sqrt(N_HIDDEN_1)))
        B2_a = tf.Variable(tf.random.uniform(
            [N_HIDDEN_2], -1 / math.sqrt(N_HIDDEN_1), 1 / math.sqrt(N_HIDDEN_1)))
        W3_a = tf.Variable(tf.random.uniform([N_HIDDEN_2, num_actions], -0.003, 0.003))
        B3_a = tf.Variable(tf.random.uniform([num_actions], -0.003, 0.003))
