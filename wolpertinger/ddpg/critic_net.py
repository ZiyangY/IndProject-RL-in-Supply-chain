...
class CriticNet:
    ...
    def __init__(self, num_states, num_actions):
        ...
        with self.g.as_default():
            ...
            self.q_value_in = tf.compat.v1.placeholder("float", [None, 1])  # supervisor
            ...
            self.action_gradients = [self.act_grad_v[0] /
                                     tf.compat.v1.to_float(tf.shape(self.act_grad_v[0])[0])]
            # from simple actor net:
    ...
    def create_critic_net(self, num_states=4, num_actions=1):
        N_HIDDEN_1 = 400
        N_HIDDEN_2 = 300
        critic_state_in = tf.compat.v1.placeholder("float", [None, num_states])
        critic_action_in = tf.compat.v1.placeholder("float", [None, num_actions])

        W1_c = tf.Variable(tf.random.uniform(
            [num_states, N_HIDDEN_1], -1 / math.sqrt(num_states), 1 / math.sqrt(num_states)))
        B1_c = tf.Variable(tf.random.uniform(
            [N_HIDDEN_1], -1 / math.sqrt(num_states), 1 / math.sqrt(num_states)))
        W2_c = tf.Variable(tf.random.uniform(
            [N_HIDDEN_1, N_HIDDEN_2], -1 / math.sqrt(N_HIDDEN_1 + num_actions), 1 / math.sqrt(N_HIDDEN_1 + num_actions)))
        W2_action_c = tf.Variable(tf.random.uniform(
            [num_actions, N_HIDDEN_2], -1 / math.sqrt(N_HIDDEN_1 + num_actions), 1 / math.sqrt(N_HIDDEN_1 + num_actions)))
        B2_c = tf.Variable(tf.random.uniform(
            [N_HIDDEN_2], -1 / math.sqrt(N_HIDDEN_1 + num_actions), 1 / math.sqrt(N_HIDDEN_1 + num_actions)))
        W3_c = tf.Variable(tf.random.uniform([N_HIDDEN_2, 1], -0.003, 0.003))
        B3_c = tf.Variable(tf.random.uniform([1], -0.003, 0.003))
