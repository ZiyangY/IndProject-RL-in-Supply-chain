...
class grad_inverter:
    def __init__(self, action_bounds):
        ...
        self.action_input = tf.compat.v1.placeholder(tf.float32, [None, self.action_size])
        ...
        self.pdiff_max = tf.compat.v1.div(-self.action_input+self.pmax, self.prange)
        self.pdiff_min = tf.compat.v1.div(self.action_input - self.pmin, self.prange)
        self.zeros_act_grad_filter = tf.zeros([self.action_size])
        self.act_grad = tf.compat.v1.placeholder(tf.float32, [None, self.action_size])
