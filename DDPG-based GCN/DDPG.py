...
class Actor(nn.Module):
    ...
    def forward(self,x,edge_index,squeeze = True, bs= 200):

        x = F.relu(self.conv1(x,edge_index))
        if squeeze:
            x = global_mean_pool(self.layer_norm(x), torch.LongTensor([0 for _ in range(len(x))]).to(device))
        else:
            x = global_mean_pool(self.layer_norm(x), torch.LongTensor([bs-1 for _ in range(len(x))]).to(device))
...
class Critic(nn.Module):
    def __init__(
        self, state_dim, state_dim_2, action_dim, hidden_dim, is_recurrent=False
    ):
    ...
    def forward(self, node ,edge_index, s,a,bs,squeeze = True):
        sa = torch.cat([s, a], -1)
        x = F.relu(self.l1(sa))
        # if is_linear:
        #     x = F.relu(self.l1(sa))
...
class DDPG(object):
    def __init__(self, state_dim, action_dim, max_action, hidden_dim, discount,
        ...
        self.actor = Actor(
            state_dim, 6,action_dim, hidden_dim, max_action,
        ).to(device)
        self.actor_target = copy.deepcopy(self.actor)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(),lr=self.actor_lr)

        self.critic = Critic(
            state_dim, 6+action_dim,action_dim, hidden_dim,
        ).to(device)
    ...
    def select_action(self, s_g, test=True):
    ...
    def train(self, replay_buffer, batch_size):
        ...
        state, state_graph, action, next_state, next_state_graph, reward, not_done = \
            replay_buffer.sample(batch_size)
            
        state_loader = DataLoader(state_graph,self.batch_size, shuffle=False)
        next_state_loader = DataLoader(next_state_graph,self.batch_size, shuffle=False)
        
        for j in next_state_loader:
            next_action = self.actor_target(j.x,j.edge_index,squeeze=False,bs = self.batch_size).detach()
            target_Q = reward + self.discount * self.critic_target(j.x,j.edge_index,next_state, next_action, bs = self.batch_size).detach()
        # print(self.gamma *self.critic_target(next_state, next_action, squeeze=False, bs = self.batch_size).detach())
        
        for j in state_loader:
            current_Q = self.critic(j.x,j.edge_index,state, action, squeeze=False, bs = self.batch_size)
        ...
        #     next_state,
        #     self.actor_target(state=next_state, bs=self.batch_size, is_linear=False),
        #      bs=self.batch_size, is_linear=True)
        # target_Q = reward + (not_done * self.discount * target_Q).detach()
        # # Get current Q estimate
        # current_Q = self.critic(state, action,  bs = self.batch_size, is_linear=False)
        ...
        # Compute actor loss
        # actor_loss = -self.critic(
        #     state, self.actor(state=state,bs=self.batch_size, is_linear=False), bs=self.batch_size, is_linear=False).mean()
        

        for j in state_loader:
             actor_loss = -self.critic(j.x,j.edge_index,state, 
                                 self.actor(j.x,j.edge_index,squeeze=False,
                                bs = self.batch_size),bs=self.batch_size).mean()
...
def create_torch_graph_data(inventory, replenishment,last_replenishment,unites_sold, lost_sales):    
    ...
    a = torch.full([len(inventory),6],0.0).to(device)

    for i in range (0, len(inventory)):
        a[i] = torch.tensor([inventory[i],replenishment[i],last_replenishment[i],
                             unites_sold[i],lost_sales[i],0])
